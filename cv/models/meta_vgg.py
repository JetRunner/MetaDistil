...
class VGG(MetaModule):
    ...
    def __init__(self, cfg, batch_norm=False, num_classes=1000):
        ...
        self.classifier = MetaLinear(512, num_classes)
    ...
    def forward(self, x, is_feat=False, preact=False, params=None):
        h = x.shape[2]
        x = F.relu(self.block0(x, params=self.get_subdict(params, 'block0')))
        f0 = x
        x = self.pool0(x)
        x = self.block1(x, params=self.get_subdict(params, 'block1'))
        f1_pre = x
        x = F.relu(x)
        f1 = x
        x = self.pool1(x)
        x = self.block2(x, params=self.get_subdict(params, 'block2'))
        f2_pre = x
        x = F.relu(x)
        f2 = x
        x = self.pool2(x)
        x = self.block3(x, params=self.get_subdict(params, 'block3'))
        f3_pre = x
        x = F.relu(x)
        f3 = x
        if h == 64:
