...
class ResNet(MetaModule):
    ...
    def forward(self, x, is_feat=False, preact=False, params=None):
        ...
        x, f1_pre = self.layer1(x, params=self.get_subdict(params, 'layer1'))  # 32x32
        f1 = x
        x, f2_pre = self.layer2(x, params=self.get_subdict(params, 'layer2'))  # 16x16
        f2 = x
        x, f3_pre = self.layer3(x, params=self.get_subdict(params, 'layer3'))  # 8x8
        f3 = x
